                ...
                handle={
                    <div className="resize-handle-container">
                        <div
                            ...
                            style={{ cursor: "ew-resize", height: "100%", width: "5px", position: "absolute", right: 0, top: 0, backgroundColor: "transparent" }}
                        />
                    </div>
                }
            >
                <div className="file-explorer" style={{ width: isCollapsed ? 40 : width }}>
                    <div className={`navbar ${isCollapsed ? "collapsed" : ""}`}>
                        {!isCollapsed && (
                            <>
                                <span className="directory-name">{currentDir}</span>
                                <button className="icon-button" title="Add File">
                                    <AddFileIcon />
                                </button>
                                <button className="icon-button" title="Add Directory">
                                    <CreateNewFolderIcon />
                                </button>
                    ...
                    {!isCollapsed && <FileExplorerContent />}
                </div>
            </ResizableBox>
        </div>
    );
...
const FileExplorerContent: React.FC = () => {
    const { response, error, loading, sendRequest } = useAxiosRequest<null, FileNode[]>();
